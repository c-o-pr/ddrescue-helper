...
SI_number_format() {
  ...
  case $(get_OS) in
    macOS)
      if ! which gnumfmt > /dev/null; then
        _error "Using SI prefix numbers requires gnumfmt(1)."
        _error "Use a plain number, or install GNU coreutils."
        exit 1
      fi
      gnumfmt --from si "$number"
      ;;
    Linux)
      numfmt --from si "$number"
      ;;
    *)
      _error "Unknown host OS"
      kill $$
...
Opt_4K=false
Opt_Zap_Slow=false
Opt_Slow_Rate=-1 # Disabled by default
Opt_Slow_Rate_Default=100000 # Bytes per second
...
while getopts ":cfhmpqsuzKMR:XZ" Opt; do
  case ${Opt} in
    c) Do_Copy=true ;;
    f) Do_Fsck=true ;;
    h) help | more; exit 0 ;;
...
#       echo XXX $Opt_Slow_Rate
       if [[ "$Opt_Slow_Rate" =~ \. ]]; then
         _error "$(basename "$0"): -R <rate> must result in an integer ($Opt_Slow_Rate)"
         exit 1
       fi
       if [ "$Opt_Slow_Rate" -le 0 ]; then
         _error "$(basename "$0"): -R <rate> must be greater than 0"
         exit 1
       fi
      ;;
    X) Opt_Scrape=true ;;
    ...
    *)
      usage
      _advise echo  "$(basename "$0"): Unknown option or missing paramter -$OPTARG"
      exit 1
      ;;
  esac
done
...
if $Do_Mount || $Do_Unmount || $Do_Fsck; then
  ...
  # XXX PERMUTATIONS OF OPTION PROCESSING ARE GROSS!
  if [ "$Opt_Slow_Rate" -ne -1 ]; then
    _error "Invalid option, -R, in context"
    exit 1
  fi
...
if \
  ...
  Label="${1%/}"
  # Target is required becuase a although a map contains a command-line record
  ...
  if ! $Preview_Zap_Regions && \
     ! is_device "$Target" && ! [ -f "$Target" ]; then
    _error "report/zap: No such device or file ($Target)"
    exit 1
  fi

  # Zap only: file path is allowed.
  # Setup path for map check beloiw.
  if $Do_Zap_Blocks && [ -f "$Target" ] && \
     ! Target=$(absolute_path "$Target"); then
    _error "zap: Invalid file path $Target"
    exit 1
  fi
  ...
  #
  if ! resource_matches_map "$Target" "$Map_File"; then
    if $Do_Error_Files_Report || $Do_Slow_Files_Report; then
      # Accept whole drive map for a partition device
      x="$(strip_partition_id "$Target")"
      if ! resource_matches_map "$x" "$Map_File"; then
        _error "report: Existing block map ($Label/$Map_File) but not for $Target"
        get_commandline_from_map "$Map_File"
        exit 1
      fi
      # Fall through
...
#      cat "$Error_Fsck_Block_List"
      fsck_device "$Target" true "$Error_Fsck_Block_List" | \
    ...
    if $Do_Slow_Files_Report; then
      ...
      # Default rate for files afftected by slow reads
...
#      exit
  ...
  if $Do_Zap_Blocks; then
    ...
    if ! $Preview_Zap_Regions; then
      ...
      trap "_abort true $Target" SIGINT SIGTERM
    fi

    zap_from_mapfile "$Target" \
                     "$Map_File" \
                     "$Zap_Block_List" \
                     $Preview_Zap_Regions \
                     $Opt_4K
